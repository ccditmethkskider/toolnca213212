...
getgenv()['Check_Setting'] = function(Name)
    if not _G.Dis and not isfolder('Sonic Hub') then
        makefolder('Sonic Hub')
    end
    if not _G.Dis and not isfile('Sonic Hub/'..Name..'.json') then
        writefile('Sonic Hub/'..Name..'.json',JsonEncode(_G.Setting_table))
    end
end
getgenv()['Get_Setting'] = function(Name)
    if not _G.Dis and isfolder('Sonic Hub') and isfile('Sonic Hub/'..Name..'.json') then
        _G.Setting_table = JsonDecode(readfile('Sonic Hub/'..Name..'.json'))
...
if IKAI then
	...
	function library:Window(text,text2,text3,logo,keybind)
		...
		Logo.Image = "rbxassetid://103731897420638"..tostring(logo)
	
		local Name = Instance.new("TextLabel")
		Name.Name = "MINHDUCDEV"
		Name.Parent = Top
		...
		ToggleImgUi.Image = "rbxassetid://103731897420638"
	...
	Yedhee.TextTransparency = 1
		...
		function uitab:Tab(text,logo1)
			...
			IDK.Image = "rbxassetid://103731897420638"..tostring(logo1)
...
game.StarterGui:SetCore("SendNotification", {
	Title = "Sonic Notification", 
	Text = "Functions with this icon can be opened\nwith the farm level.",
	Icon = "http://www.roblox.com/asset/?id=9610159123",
	Duration = 8
})

local win = library:Window("Sonic",[[Hub]],[[ ST ]],"9606070311",Enum.KeyCode.RightControl)
...
General_Tab:Toggle("Fast Attack ( RECOMMENDED ) ","9606294253",function()
(getgenv()).Config = {
 ["FastAttack"] = true,
 ["ClickAttack"] = false
} 
		...
		pcall(function()
			if Auto_Three_World then
					...
					if game:GetService("Workspace").Enemies:FindFirstChild("rip_indra [Lv. 1500] [Boss]") then
						for i,v in pairs(game.Workspace.Enemies:GetChildren()) do
							if v.Name == "rip_indra [Lv. 1500] [Boss]" and v.Humanoid.Health > 0 then
								...
								if not YUHJO then
									Text("Complete the World 3 quest.")
									YUHJO = true
								end
							end
						end
				...
				elseif game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("TalkTrevor","1") == 0 then
					if not IJOIJD then
						Text("You have completed the World 3 quest.")
						IJOIJD = true
					end
				else
					if not JUO then
						Text("You haven't opened the Dofa door yet.\nCan't do that.้!!")
						JUO = true
					end
				end
			else
				...
				if not Mix_Farm or Auto_Phoenix_Awaken_Ex then
					if game.Players.LocalPlayer.Character:FindFirstChild("Bird-Bird: Phoenix") or game.Players.LocalPlayer.Backpack:FindFirstChild("Bird-Bird: Phoenix") then
						if game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("SickScientist","Check") ~= false then
							if not UHuHI then
								Text("Cultivating Devil Fruit Mastery")
								UHuHI = true
							end
							Auto_Farm_Fruit = true
							...
							if game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("SickScientist","Heal") == 1 then
								...
								if FG >= 6000 then
									...
									if _G.Have_Fruit then
										...
										elseif _G.Setting_table.Phoenix_Hop then
											if not UHIkx then
												Text("Wait 2 hours before you can buy chips to push.\nPurple money farm, better wait.")
												UHIkx = true
											end
											_G.Phoenix_R = true
											Raid_FG()
											...
											if not UHIkz then
												Text("Wait 2 hours before you can buy chips.")
												UHIkz = true
											end
										end
									elseif Beli >= 1000000 then
										...
										elseif _G.Setting_table.Phoenix_Hop then
											...
											if not UHIk then
												Text("Wait 2 hours to buy chips.้")
												UHIk = true
											end
										end
									elseif Auto_Phoenix_Awaken_Ex then
										...
										Text("is pouring money 1 million\nto buy a chip to push phoenix")
										wait(10)
									end
								elseif Auto_Phoenix_Awaken_Ex then
									Auto_Phoenix_Awaken_Ex = nil
									...
									_G.Phoenix_R = true
									Raid_FG()
									Text("Purple money doesn't arrive 6000\nUnable to push้")
									Text("looking for results!")
									wait(10)
								end
							elseif FG < 1500 then
								if not LPOJO then
				...
				if game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("TalkTrevor","1") == 0 then
					if game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Alchemist","1") ~= -2 then
						...
						elseif game.Players.LocalPlayer.Backpack:FindFirstChild("Flower 3") and game.Players.LocalPlayer.Backpack:FindFirstChild("Flower 1") and game.Players.LocalPlayer.Backpack:FindFirstChild("Flower 2") then
							...
							if not UHuit then
								Text("waiting for the flower to be born")
								TP2(CFrame.new(-379.70889282227, 73.0458984375, 304.84692382813))
								UHuit = true
							end
							wait(2)
					...
					Text("The Dofa door hasn't opened yet.\nClans can't be made.")
					wait(5)
				end
