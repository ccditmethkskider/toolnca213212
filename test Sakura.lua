...
SpeedSection:Slider({
    ...
    Content = "Sử dụng thanh trượt để điều chỉnh tốc độ chạy.",
    ...
    Callback = function(value)
        ...
        if speedToggle then
            local player = game.Players.LocalPlayer
            player.Character.Humanoid.WalkSpeed = currentWalkSpeed
            print("Tốc độ đã được điều chỉnh:", currentWalkSpeed)
        end
    end
})
...
local EspSection = FunctionTab:Section({
    ...
    Content = "Hiện thị vị trí của tất cả người chơi."
})
...
local function createESP(player)
    if player.Character and not espHighlights[player.UserId] then
        ...
        highlight.OutlineColor = Color3.fromRGB(0, 0, 0) -- Màu viền đen
