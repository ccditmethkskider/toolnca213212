...
local Converted = {
	["_FREE FIRE LITE"] = Instance.new("ScreenGui");
...
Converted["_YARHM"].Name = "FREE FIRE LITE"
Converted["_YARHM"].Parent = game:GetService("CoreGui")

Converted["_Open"].Font = Enum.Font.Gotham
Converted["_Open"].Text = "Triple-click this region to open FF."
...
Converted["_Open"].Parent = Converted["_FREE FIRE LIET"]
...
Converted["_ListButton"].Parent = Converted["_FREE FIRE LITE"]
...
Converted["_Notifications"].Parent = Converted["_FREE FIRE LITE"]
...
Converted["_TextBoxPlaceholder"].Parent = Converted["_FREE FIRE LITE"]
...
Converted["_FloatingButton"].Parent = Converted["_FREE FIRE LITE"]
...
Converted["_FloatingButtons"].Parent = Converted["_FREE FIRE LITE"]
...
Converted["_AddCustomModule"].Parent = Converted["_FIRE FIRE LITE"]
...
Converted["_DroppedGunBGUI"].Parent = Converted["_FREE FIRE LITE"]
...
Converted["_MurdererBGUI"].Parent = Converted["_FREE FIRE LITE"]
...
Converted["_Menu"].Parent = Converted["_FREE FIRE LITE"]
...
Converted["_HubName"].Text = "FREE FIRE LITE"
...
Converted["_ImageLabel"].Image = "rbxassetid://127393374937080"
...
Converted["_TextLabel7"].Text = "FREE FIRE LITE"
...
Converted["_Toggle"].Parent = Converted["_FIRE LITE"]
...
Converted["_ImageLabel1"].Image = "rbxassetid://127393374937080"
...
Converted["_Modules"].Parent = Converted["_FREE FIRE LITE"]
...
Converted["_DropdownFrameSample"].Parent = Converted["_FREE FIRE LITE"]
...
Converted["_Range"].Parent = Converted["_FREE FIRE LITE"]
...
Converted["_Dialog"].Parent = Converted["_FREE FIRE LITE"]
...
Converted["_NotificationSample"].Parent = Converted["_FREE FIRE LITE"]
...
do -- Fake Module: ServerStorage.YARHM.FUNCTIONS
    ...
    script.Parent = Converted["_FREE FIRE LITE"]
    local function module_script()
		...
		function FUNCTIONSmodule.refreshlist()
			...
			for modulei, module in ipairs(_G.Modules) do
				...
				if not success then
					warn("[FREE FIRE] Error while requiring " .. module.Name .. "!")
					warn(err)
				end
			end
		end
...
local function IYGQL_fake_script() -- Fake Script: ServerStorage.YARHM.Init
	...
	local s, e = pcall(function()
		if get_hidden_gui or gethui then
			...
			print("[FREE FIRE] - Using get_hidden_gui for anti-detection.")
		elseif (not is_sirhurt_closure) and (syn and syn.protect_gui) then
			script.Parent.Name = randomString()
			syn.protect_gui(script.Parent)
			script.Parent.Parent = COREGUI
			print("[FREE FIRE] - Using syn.protect_gui for anti-detection.")
		elseif COREGUI:FindFirstChild('RobloxGui') then
			script.Parent.Parent = COREGUI.RobloxGui
			print("[FREE FIRE] - Using RobloxGui for anti-detection.")
		else
			warn("[FREE FIRE] - Using CoreGui as anti-detection. This is the most basic coverage and can still be detected.")
		end
	end)
	
	print("[FREE FIRE] - FREE FIRE is now in " .. tostring(script.Parent:GetFullName()))
	if not s then
		warn("[FREE FIRE] - Attempts at anti-detection failed. Using CoreGui as anti-detection.")
		warn(e)	
	end
	
	printidentity("[FREE FIRE] - Your executor level (identity) is")
	
	local getExeName = identifyexecutor or getexecutorname or function() return "Yet Another Roblox Executor v1.2" end
	print("[FREE FIRE] - Your executor is " .. getExeName())
	...
	require(script.Parent.FUNCTIONS).notification("Welcome to FREE FIRE LITE.")
			...
			Auto_Shoot_murderer = function()
				...
				if findSheriff() == localplayer and autoShooting then
					...
					repeat
						task.wait(0.1)
						local murderer = findMurderer() or findSheriffThatsNotMe()
						if not murderer then warn("[FREE FIRE] > MM2 Autoshoot - No murderer.") continue end
						local murdererPosition = murderer.Character.HumanoidRootPart.Position
						local characterRootPart = localplayer.Character.HumanoidRootPart
...
local function PLIORD_fake_script() -- Fake Script: ServerStorage.YARHM.Universal
	...
	table.insert(module, {
		Type = "Text",
		Args = {"Welcome to FREE FIRE! The open, free script hub."}
	})
	...
	table.insert(module, {
		...
		Args = {"Join our Discord", function(Self)
			if setclipboard then setclipboard("https://discord.gg/48A4gJmEFN") end
	...
	table.insert(module, {
		...
		Args = {"Hide +", function(Self)
			...
			elseif uis.AccelerometerEnabled then
				...
				fu.notification("Can't hide FREE FIRE!") -- how else are you gonna open???
			end
		end,}
	}
	)
	...
	local usernameColors = {
		...
		["freefireplus"] = Color3.fromHex("#f48fff"),
	...
	local mainText = "Duc Developer"
	local mainFont = Enum.Font.GothamBold
	local specialText = "duc+"
	...
	table.insert(module, {
		Type = "Toggle",
		Args = {"Hide Duc+/Developer tags", function(Self, state)
