...
local Tabs = {
    infor = Window:AddTab({ Title = "Discord", Icon = "" }),
    Home = Window:AddTab({ Title = "Trạng Thái", Icon = "" }),
    Main = Window:AddTab({ Title = "Tab Chính", Icon = "" }),
    item = Window:AddTab({ Title = "Vật phẩm", Icon = "" }),
    C = Window:AddTab({ Title = "Tab Combat", Icon = "" }),
    tele = Window:AddTab({ Title = "Tele Đảo", Icon = "" }),
...
ImageButton.Image = "http://www.roblox.com/asset/?id=137961660931914"
...
Tabs.infor:AddButton({
        ...
        Callback = function()
        setclipboard("https://discord.gg/TmPK6ZxJ")
        end
    })
---------------TabStatus
    ...
    while wait() do
        spawn(
            ...
            or game:GetService("Workspace").Enemies:FindFirstChild("Urban") then
                ConMeMayThangWidiBuCacAnhDi:SetDesc(" Đã Xuất Hiện")	
            else
                ConMeMayThangWidiBuCacAnhDi:SetDesc(" Hiện không có")	
            end
        end
        )
    end
...
local function updateMirageStatus()
    ...
    if mirageIsland then
        DaoNaoCac:SetDesc('Status: Server đang có đảo')
    else
        DaoNaoCac:SetDesc('Status: Hiện chưa xuất hiện')
    end
end
...
local Toggle = Tabs.Main:AddToggle("Bring Mob", { Title = "Gom quái (vip)", Default = true })
...
local Toggle = Tabs.Main:AddToggle("Bypass TP", { Title = "Dừng Bypass khi có key và chén", Default = false })
Toggle:OnChanged(function(Value)
    _G.StopTP = Value
end)
...
local Dropdown = Tabs.Main:AddDropdown("DropdownFarm", {
    ...
    Values = {"Farm Cấp", "Farm Xương", "Farm Katakuri"},
    Multi = false,
})
...
local ConCac = Tabs.Main:AddSection("Farm Mics Theo Sea")

if AnDepZai3 then
local Dropdown = Tabs.Main:AddDropdown("DropdownFarm", {
    Title = "Chọn Item Farm ( Sea 3 )",
    Values = {"Cacao", "Vảy rồng", "Da"},
    Multi = false,
})
...
local Toggle = Tabs.item:AddToggle("Auto Ectoplasm", { Title = "Mua kiếm của Zoro", Default = false })
...
local Toggle = Tabs.item:AddToggle("auto Kill", { Title = "Auto Rip_Indra", Description = "Auto win 10 thằng Rip_Indra", Default = false })
Toggle:OnChanged(function(Value)
...
local Toggle = Tabs.item:AddToggle("auto Kill", { Title = "Giết Boss Longma", Default = false })
Toggle:OnChanged(function(Value)
...
local Toggle = Tabs.item:AddToggle("Auto Eliter Hunter", { Title = "tự động giết eliter", Default = false })
Toggle:OnChanged(function(Value)
    _G.Eli_Ngu = Value
game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("AbandonQuest")
...
local Toggle = Tabs.item:AddToggle("Auto Eliter Hunter", { Title = "Tự động đánh hải tặc", Default = false })
Toggle:OnChanged(function(Value)
    _G.RaidPirate = Value
...
local Toggle = Tabs.item:AddToggle("auto Change", { Title = "Giết boss Soul Reaper", Description = "", Default = false })
Toggle:OnChanged(function(Value)
_G.Buoi = Value
StopTween(_G.Buoi)
end)         
...
local Toggle = Tabs.item:AddToggle("auto Change", { Title = "Tự động Dough King + Farm Item", Description = "Tự động cân thằng Dough", Default = false })
Toggle:OnChanged(function(Value)
...
local ToggleTeleport = Tabs.C:AddToggle("ToggleTeleport", {Title = "Bay đến người chơi", Description = "",Default = false })
...
local Mastery = Tabs.tele:AddSection("Bay đến đảo")

if AnDepZai1 then
local Dropdown1 = Tabs.tele:AddDropdown("DropdownFarm", {
    Title = "Chọn đảo",
    Values = {"Cối xoay", "Hải quân","Middle Town", "Rừng", "Làng hải tặc", "Sa mạc", "Đảo tuyết", "MarineFord", "Đấu trường", "Đảo trời 1", "Đảo trời 2", "Đảo trời 3", "Nhà tù", "Làng magma", "Đảo dưới nước","Đài phun nước", "Shank Room"},
    Multi = false,
})
...
local Dropdown3 = Tabs.tele:AddDropdown("DropdownFarm3", {
    Title = "chọn đảo",
    Values = {"Dinh thự","Thị trấn","Đảo cây","Pháo đài","Đảo phụ nữ","Lâu đài bóng tối","Đảo kem","Đảo đậu phộng","Đảo bánh","Đảo cacao","Đảo kẹo","Thành phố Tiki"},
    Multi = false,
})
...
local Toggle = Tabs.an:AddToggle("Auto Spawn Boss Kata", { Title = "Tự động Spawn Boss Katakuri", Default = true })
Toggle:OnChanged(function(Value)
...
local Toggle = Tabs.an:AddToggle("Walk Water", { Title = "Đứng trên nước", Default = true })
Toggle:OnChanged(function(Value)
    _G.NuocLon = Value
end)
...
local Toggle = Tabs.an:AddToggle("Walk Water", { Title = "Tự động join lại", Description = "Rejoin When Kick", Default = false })
Toggle:OnChanged(function(Value)
    _G.Rejoin = Value
end)                          
...
local Toggle = Tabs.an:AddToggle("RemoveConMe", { Title = "Remove Notify", Description = "Xóa thông báo Farm", Default = false })
Toggle:OnChanged(function(AnDepZaiHubOnTop)
...
Tabs.an:AddButton({
    Title = "Mở shop trên đảo bí ẩn",
    Description = "Chỉ có thể nhìn thôi",
    Callback = function()
...
local Toggle = Tabs.Race:AddToggle("Teleport Gear", { Title = "Tìm ông già bán fruit", Default = false })
Toggle:OnChanged(function(Value)
    _G.RaDaoKhi = Value
...
Tabs.Race:AddButton({
    ...
    Description = "Bay Đến Chỗ Gắn Gear",
    Callback = function()
...
local Toggle = Tabs.Race:AddToggle("Auto Complete Trial", { Title = "Tự động Train+Mua Gear", Default = false })
...
local Toggle = Tabs.Race:AddToggle("Auto Complete Trial", { Title = "Tự động win ải", Default = false })
...
local Toggle = Tabs.Raid:AddToggle("Auto Buy Chip", { Title = "Tự động mua Chip", Default = false })
Toggle:OnChanged(function(Value)
...
local Toggle = Tabs.Melee:AddToggle("Random Bones", { Title = "Random Xương", Default = false })
